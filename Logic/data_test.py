...
# check data types
    for movie in data:
        for field, expected_type in expected_fields.items():
            assert field in movie, f'Expected field {field} not found in movie {movie["id"]}'
            if expected_type is not None:
                assert isinstance(movie[field], expected_type), f'Expected field {field} to be of type {expected_type}, but got {type(movie[field])} in movie {movie["id"]}'
expected_fields = {
    'id': str,
    'title': str,
